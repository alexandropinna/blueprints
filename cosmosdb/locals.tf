locals {
  ...
  # Nomenclature: cosmos-<cl>-<env>-<reg>-<stamp>-<inst>
  name = "cosmos-${local.client_code}-${local.environment}-${local.region_short}-${var.stamp}-${local.instance_number}"

  # Validated name (max 50 characters for Cosmos DB account)
