locals {
  ...
  # Nomenclature: logic-<cl>-<env>-<reg>-<comp>-<stamp>-<inst>
  name = "logic-${local.client_code}-${local.environment}-${local.region_short}-${var.component}-${var.stamp}-${local.instance_number}"

  # Validated name (max 80 characters for logic app)
